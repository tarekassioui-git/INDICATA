<?php 
    ...
    {	
        ...
        $sql = 'INSERT INTO `gestionale`.wp_cars (targa, marca, modello, carburante, potenza, cambio, immatricolazione, trazione, telaio)
        VALUES ("' . $data['targa'] . '", 
            "' . $data['marca'] . '",
            "' . $data['modello'] . '",
            "' . $data['carburante'] . '",
        ...
        for($i = 0; $i < sizeof($data['versione']); $i++)
        {
            ...
            VALUES ("' . $data['targa'] . '", 
                "' . json_encode(array('versione' => $data['versione'][$i]['versione'],'codice' => $data['versione'][$i]['codice'] )) . '")';
